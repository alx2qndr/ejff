...
RenderPass::RenderPass(
    ...
    const SDL_GPUColorTargetInfo *colorTargetInfos, uint32_t numColorTargets,
...
{
    auto renderPass =
        SDL_BeginGPURenderPass(commandBuffer.get(), colorTargetInfos,
                               numColorTargets, depthStencilTargetInfo);
    if (!renderPass)
    {
        throw std::runtime_error(
            fmt::format("Couldn't begin SDL_GPURenderPass. "
